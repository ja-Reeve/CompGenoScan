...
### diversity of two populations of threespine stickleback.
...
### Download VCF
Ts.vcf <- read.table("file:///F:/vcf-files/Ts.VarScan.vcf", skip =  23) # Skip 23 lines of header
colnames(Ts.vcf) <- c("CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO", "FORMAT", "TsAK", "TsOR")
Ts.vcf <- Ts.vcf[Ts.vcf$CHROM != "chrM" & Ts.vcf$CHROM != "chrUn",]
...
for(i in 1:length(chrom_list)){
  Ts.vcf[Ts.vcf$CHROM == chrom_list[i], "CHROM"] <- new_chrom_list[i]
}
...
nucleotide.diversity.window <- function(data, sample.name, window.size, list.chromosomes, chromosome.max.length){
  ...
  for (i in 1:length(list.chromosomes)) {
    ...
    for(j in 1:Nwinds){
      ...
      results[j,5] <- paste0("Ts_", list.chromosomes[i], ":", wind_start, "-", wind_end) # Window ID
...
# Northen population
TsAK.He <- nucleotide.diversity.window(data = Ts.vcf, sample.name = "TsAK", window.size = 50000,
                                     list.chromosomes = chrom_list, chromosome.max.length = max_length)
# Southern population
TsOR.He <- nucleotide.diversity.window(data = Ts.vcf, sample.name = "TsOR", window.size = 50000,
                                     list.chromosomes = chrom_list, chromosome.max.length = max_length)
# Merge into single dataset
Ts.He <- data.frame("CHROM" = TsAK.He$CHROM, 
                    "START_POS" = TsAK.He$START_POS, "END_POS" = TsAK.He$END_POS,
                    "WIND" = TsAK.He$WIND, 
                    "TsAK_He" = TsAK.He$He, "TsOR_He" = TsOR.He$He,
                    "Avg_He" = rowMeans(cbind(TsAK.He$He, TsOR.He$He)))

###### Note: last window for each chromosome is underestimate becasue of incomplete window ###

### Save data
write.table(Ts.He, "file:///F:/nucleotide_diversity/Ts_Avg_Hexp_50Kwind.txt", row.names = FALSE, quote = FALSE)

### Plots
#Plot TsAK vs TsOR
Ts.He$Outlier <- Ts.He$Avg_He < quantile(Ts.He$Avg_He, 0.01) | Ts.He$Avg_He > quantile(Ts.He$Avg_He, 0.99)

ggplot(Ts.He)+
  geom_point(aes(x = TsAK_He, y = TsOR_He, colour = Outlier), size = 0.1)+
  geom_hline(yintercept = quantile(Ts.He$TsOR_He, 0.99), lty = 2, colour = "firebrick")+
  geom_hline(yintercept = quantile(Ts.He$TsOR_He, 0.01), lty = 2, colour = "firebrick")+
  geom_vline(xintercept = quantile(Ts.He$TsAK_He, 0.99), lty = 2, colour = "firebrick")+
  geom_vline(xintercept = quantile(Ts.He$TsAK_He, 0.01), lty = 2, colour = "firebrick")+
  labs(x = expression(TsAK~bar(H)[E]), y = expression(TsOR~bar(H)[E]))+
...
Plots <- list()  
for(i in 1:length(chrom_list)){
  Plots[[i]] <- He.plot.chrom(data = Ts.He[Ts.He$CHROM == chrom_list[i],], title = chrom_list[i])
}

# Multi panel plot
Ts.pts <- grid.arrange(grobs = Plots[c(1:4,6:9,5,10:14,16:19,15,20:21)], ncol = 7, nrow = 3,
                       top = "Threespine stickleback", left = "Genetic diversity (He)", bottom = "Mid point of window (Mb)")
