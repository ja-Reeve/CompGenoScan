...
#-o $DATA_HOME/TuBC_Indel.vcf \
#-out_mode EMIT_ALL_SITES \
#-ploidy 100 \
#| $BCFTOOLS view -v indels $DATA_HOME/TuBC_Indel.vcf >> $LOG_PATH/TuBC.IndelCall.log 2>&1
...
-R $DATA_REF/tubesnout_genome.v2.fasta \
-I $DATA_BAM/TuBC_DupMark.bam \
-o $DATA_HOME/TuBC_realignment_sites.list >> $LOG_PATH/TuBC.RealigmentSet.log 2>&1

## Realign Reads
java -XX:ParallelGCThreads=4 -Xmx16G -Djava.io.tmpdir=/data/home/james/tmp -jar $GATK3 \
-T IndelRealigner \
-R $DATA_REF/tubesnout_genome.v2.fasta \
-targetIntervals $DATA_HOME/TuBC_realignment_sites.list \
-I $DATA_BAM/TuBC_DupMark.bam \
-o $DATA_BAM/TuBC_Realign.bam >> $LOG_PATH/TuBC.IndelRealigner.log 2>&1

## Error Check
java -XX:ParallelGCThreads=4 -Xmx16G -Djava.io.tmpdir=/data/home/james/tmp -jar $PICARD ValidateSamFile \
      I=$DATA_BAM/TuBC_Realign.bam \
      MODE=SUMMARY >> $LOG_PATH/TuBC.ValidateRealignBam.log 2>&1
