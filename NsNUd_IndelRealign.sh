...
-R $DATA_REF/ninespine_genome_draft.fasta \
-I $DATA_BAM/NsNUd_DupMark.bam \
-o $DATA_HOME/NsNUd_realignment_sites.list >> $LOG_PATH/NsNUd.RealigmentSet.log 2>&1

## Realign Reads
java -XX:ParallelGCThreads=16 -Xmx32G -Djava.io.tmpdir=/data/home/james/tmp -jar $GATK3 \
-T IndelRealigner \
-R $DATA_REF/ninespine_genome_draft.fasta \
-targetIntervals $DATA_HOME/NsNUd_realignment_sites.list \
-I $DATA_BAM/NsNUd_DupMark.bam \
-o $DATA_BAM/NsNUd_Realign.bam >> $LOG_PATH/NsNUd.IndelRealigner.log 2>&1

## Error Check
java -XX:ParallelGCThreads=16 -Xmx32G -Djava.io.tmpdir=/data/home/james/tmp -jar $PICARD ValidateSamFile \
      I=$DATA_BAM/NsNUd_Realign.bam \
      MAX_OPEN_TEMP_FILES=1000 \
      MODE=SUMMARY >> $LOG_PATH/NsNUd.ValidateRealignBam.log 2>&1
