...
### latitute adaptation between two populations of tubesnout.
...
gene_anno <- read.table("file:///F:/GFF/Tu_gene_anno.txt", header = TRUE)
...
top_candidate_test<-function(data, window_size, binomial_cutoff, threshold, chromosome_max){
  ...
  for (i in 1:length(chrom_list)) {
    ...
    for(t in 1:Nwinds){
      ...
      results[t,4] <-paste0("Tu_", chrom_list[i], ":", wind_start, "-", wind_end) #Window ID
...
Cands <- top_candidate_test(Tu.Fst, 50000, 0.9999, 0.999, max_length)

### Save the data
write.table(Cands, "file:///F:/top-candidate/Tu.TopCandidates.50Kwind.txt", row.names = FALSE, quote = FALSE)
...
ggplot(Cands %>% arrange(SNPs_in_Window))+
  ...
  ggtitle("Tubesnout: B = 0.9999 Q = 999th")+
