...
#-o $DATA_HOME/TsOR_Indel.vcf \
#-out_mode EMIT_ALL_SITES \
#-ploidy 104 \
#| $BCFTOOLS view -v indels $DATA_HOME/TsOR_Indel.vcf >> $LOG_PATH/TsOR.IndelCall.log 2>&1
...
-R $DATA_REF/threespine_genome.fa \
-I $DATA_BAM/TsOR_DupMark.bam \
-o $DATA_HOME/TsOR_realignment_sites.list >> $LOG_PATH/TsOR.RealigmentSet.log 2>&1

## Realign Reads
java -XX:ParallelGCThreads=4 -Xmx16G -Djava.io.tmpdir=/data/home/james/tmp -jar $GATK3 \
-T IndelRealigner \
-R $DATA_REF/threespine_genome.fa \
-targetIntervals $DATA_HOME/TsOR_realignment_sites.list \
-I $DATA_BAM/TsOR_DupMark.bam \
-o $DATA_BAM/TsOR_Realign.bam >> $LOG_PATH/TsOR.IndelRealigner.log 2>&1

## Error Check
java -XX:ParallelGCThreads=4 -Xmx16G -Djava.io.tmpdir=/data/home/james/tmp -jar $PICARD ValidateSamFile \
      I=$DATA_BAM/TsOR_Realign.bam \
      MODE=SUMMARY >> $LOG_PATH/TsOR.ValidateRealignBam.log 2>&1
