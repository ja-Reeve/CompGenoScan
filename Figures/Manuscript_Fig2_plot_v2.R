...
### Ninespine SNPs are plotted on the Threespine genome
...
### C: Map ninespine scores onto threespine genome ####
...
### Multi-panel plot
labsY <- textGrob(expression(F[ST]), rot = 90, gp = gpar(fontsize = 20))
p1 <- grid.arrange(Ts.Manhat.Fst, Ns.Manhat.Fst, Tu.Manhat.Fst, ncol = 1, left = labsY)

labsY <- textGrob(expression(bar(H[E])), rot = 90, gp = gpar(fontsize = 20))
labsX <- textGrob("Position", gp = gpar(fontsize = 18))
p2 <- grid.arrange(Ts.Manhat.He, Ns.Manhat.He, Tu.Manhat.He, ncol = 1, left = labsY, bottom = labsX)

Fig2 <- plot_grid(p1, p2, labels = "AUTO", label_size = 24, ncol = 1)
